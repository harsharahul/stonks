...
const Kbd: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <kbd className="inline-flex items-center justify-center min-w-[24px] px-1.5 py-0.5 bg-neutral-100
                  text-neutral-700 font-mono text-xs rounded border border-neutral-200">
    {children}
  </kbd>
);
...
const sections = [
  {
    title: 'Search',
    shortcuts: [
      { keys: <><Kbd>Ctrl</Kbd><span className="text-neutral-400">+</span><Kbd>K</Kbd></>, label: 'Open search' },
  ...
  },
  {
    title: 'Navigation',
    shortcuts: [
      { keys: <><Kbd>Ctrl</Kbd><span className="text-neutral-400">+</span><Kbd>1</Kbd></>, label: 'Dashboard' },
      { keys: <><Kbd>Ctrl</Kbd><span className="text-neutral-400">+</span><Kbd>2</Kbd></>, label: 'Stocks' },
      { keys: <><Kbd>Ctrl</Kbd><span className="text-neutral-400">+</span><Kbd>3</Kbd></>, label: 'AI Intelligence' },
...
const KeyboardShortcutsHelp: React.FC<KeyboardShortcutsHelpProps> = ({ open, onOpenChange }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100]" onClick={() => onOpenChange(false)}>
      ...
      {/* Modal */}
      <div
        className="fixed top-[15%] left-1/2 -translate-x-1/2 w-full max-w-md bg-white rounded-xl shadow-2xl p-6"
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="p-1 rounded-md hover:bg-neutral-100 text-neutral-500 hover:text-neutral-700 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {sections.map((section) => (
          <div key={section.title}>
            <h3 className="text-xs font-semibold text-neutral-500 uppercase tracking-wider mt-4 mb-2">
              {section.title}
            </h3>
