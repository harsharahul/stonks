...
function getRsiZone(rsi: number): { label: string; color: string } {
  ...
  if (rsi >= 60) return { label: 'Strong', color: 'text-orange-500' };
  if (rsi >= 40) return { label: 'Neutral', color: 'text-neutral-500' };
  if (rsi >= 30) return { label: 'Weak', color: 'text-blue-500' };
  return { label: 'Oversold', color: 'text-green-600' };
}

function getMacdStatus(macd: number, signal: number): { label: string; color: string } {
  if (macd > signal) return { label: 'Bullish crossover', color: 'text-green-600' };
  return { label: 'Bearish crossover', color: 'text-red-600' };
}

function getBbPosition(close: number, upper: number, lower: number): { pct: number; label: string; color: string } {
  const range = upper - lower;
  if (range === 0) return { pct: 50, label: 'Mid-band', color: 'text-neutral-500' };
  const pct = Math.round(((close - lower) / range) * 100);
  if (pct >= 85) return { pct, label: 'Near upper band', color: 'text-red-500' };
  if (pct >= 60) return { pct, label: 'Upper half', color: 'text-orange-500' };
  if (pct >= 40) return { pct, label: 'Mid-band', color: 'text-neutral-500' };
  if (pct >= 15) return { pct, label: 'Lower half', color: 'text-blue-500' };
  return { pct, label: 'Near lower band', color: 'text-green-600' };
}
...
const LatestCandleInsight: React.FC<LatestCandleInsightProps> = ({ data, ticker, className }) => {
  ...
  const sentimentBorderColor =
    marketRead.sentiment === 'bullish' ? 'border-green-500' :
    marketRead.sentiment === 'bearish' ? 'border-red-500' : 'border-amber-500';
    ...
    <div className={cn('card', className)}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold text-neutral-900">Latest Candle Insight</h3>
        </div>
        <span className="text-sm text-neutral-500">{dateStr} &middot; {ticker}</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="flex gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <div className="space-y-0.5 text-xs text-neutral-500">
              <div>Body: <span className="font-medium text-neutral-700">{candle.bodyPct}%</span></div>
              <div>Upper wick: <span className="font-medium text-neutral-700">{candle.upperWickPct}%</span></div>
              <div>Lower wick: <span className="font-medium text-neutral-700">{candle.lowerWickPct}%</span></div>
            </div>
            {changePct !== null && (
              <div className={cn(
        ...
        {/* Column 2: Patterns */}
        <div>
          <div className="text-xs font-semibold text-neutral-400 uppercase tracking-wide mb-2">Patterns</div>
          {data.patterns.length > 0 ? (
                  ...
                  <div key={i}>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={cn(
                        ...
                        p.sentiment === 'bullish' ? 'bg-green-50 text-green-700' :
                        p.sentiment === 'bearish' ? 'bg-red-50 text-red-700' :
                        'bg-amber-50 text-amber-700'
                      )}>
                        {p.sentiment}
                      </span>
                    </div>
                    <p className="text-xs text-neutral-500 ml-7">{p.description}</p>
                    {signal && (
                      <p className="text-xs text-neutral-700 ml-7 mt-0.5 italic">{signal}</p>
                    )}
                  </div>
                );
              })}
            ...
            <div>
              <p className="text-sm text-neutral-500">No candlestick pattern detected</p>
        ...
        {/* Column 3: Indicators */}
        <div>
          <div className="text-xs font-semibold text-neutral-400 uppercase tracking-wide mb-2">Indicators</div>
          <div className="space-y-3">
            {/* RSI */}
            <div>
            ...
            <div>
              <div className="flex items-center justify-between text-xs mb-0.5">
                <span className="text-neutral-500">Bollinger Bands</span>
      ...
      {/* Market Read bar */}
      <div className={cn(
        'mt-4 p-3 rounded-r-lg border-l-4 bg-neutral-50 text-sm',
        sentimentBorderColor
      )}>
        <span className="font-semibold text-neutral-700">Market Read: </span>
        <span className="text-neutral-600">{marketRead.text}</span>
      </div>
    </div>
  );
};
