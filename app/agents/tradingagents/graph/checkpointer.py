...
# PATCH (Stonks): ``langgraph-checkpoint-sqlite`` is a heavy optional dep we
# don't pull into our requirements. Stonks runs the desk graph without
# checkpointing (config flag ``checkpoint_enabled=False``), so this import
# is unused. Make it conditional so module load succeeds without the dep.
try:
